...
export default function CourseForm({ recordForEdit, callback }) {
  const SubmitCourseSchema = Yup.object().shape({
    courseName: Yup.string().max(22, 'Must be 22 characters or less').required('required'),
    price: Yup.number().min(0).required('Should great or equal 0')
  });
  const comCode = getComCode();
  const formik = useFormik({
    initialValues: {
      courseName: recordForEdit ? recordForEdit.courseName : '',
      courseType: recordForEdit ? recordForEdit.courseType : '1:N',
      price: recordForEdit ? recordForEdit.price : 0,
      enabled: recordForEdit ? recordForEdit.enabled : true
    },
    validationSchema: SubmitCourseSchema,
    onSubmit: (values, { setSubmitting }) => {
      let url = '';
      let method = {};
      if (recordForEdit) {
        url = `/courses/${comCode}/${recordForEdit.courseId}`;
        method = http.put;
      } else {
        url = `/courses/${comCode}/addCourse`;
        method = http.post;
      }
      method(url, {
        comCode,
    ...
    <FormikProvider value={formik}>
      <Form autoComplete="off" noValidate onSubmit={handleSubmit}>
        <Stack spacing={3}>
          ...
          />
          <FormControl disabled={!!recordForEdit}>
            <FormLabel>Course Type</FormLabel>
            <RadioGroup column value={values.courseType} {...getFieldProps('courseType')}>
              {[
                { id: '1:1', title: '1 teacher for 1 student' },
                { id: '1:N', title: 'One teacher for multi students' },
                { id: 'N:N', title: 'Multi teachers for multi students' },
                { id: 'N:1', title: 'Multi teachers for 1 student' }
              ].map((item, index) => (
                <FormControlLabel
                  key={item.id}
                  value={item.id}
